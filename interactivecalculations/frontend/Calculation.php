...
class Calculation {
	...
	public static function getConfiguredCalculationAsOptions() {
		$allCalculations = \intercalcus\db\model\IcalculationsDescription::get_all();
		$options         = [ "default_intercalcus_option0" => __( '-- NONE --', 'intercalcus' ) ];
	...
	public function parseCalculationData() {
		$chosenCalculation = \intercalcus\db\model\IcalculationsDescription::get( "id", $this->calculationId );
	...
	public function render(): string {
		$calculationBlock = '<div id="intercalcus-calculation-' . $this->calculationId . '" class="intercalcus-calculation-wrapper">';
	...
	private function displayTitle(): string {
		...
		return '<h3 class="intercalcus-calculation-title">' . $this->calculationName . '</h3>';
	}
	...
	private function appendScrips(): string {
		...
		if ( ! $appendScripts->isEmpty() ) {
			...
			$interactionScript = "let " . $this->uniqueName( 'intercalcus-calculation-' . $this->calculationId ) . " = document.getElementById('intercalcus-calculation-" . $this->calculationId . "');" .
			                     "intercalcus_register_interactions(" . $this->uniqueName( 'intercalcus-calculation-' . $this->calculationId ) . "," . $this->calculationId . ");";
	...
	const INTERCALCUS_COMPONENTS_WITH_NO_ONCHANGE = array(
		"sum",
	...
	private function addOnChangeListenerToSum( $component ): string {
		...
		if ( in_array( $cleanedType, Calculation::INTERCALCUS_COMPONENTS_WITH_NO_ONCHANGE ) ) {
			return "";
			...
			return
				...
				"var " . $this->uniqueName( $component->get_dom_id() ) . " = document.getElementById('" . $component->get_dom_id() . "')}
					" . $this->uniqueName( $component->get_dom_id() ) . ".addEventListener(\"change\", function() {
					    ...
					    intercalcus_update_pre_and_calculation('" . $this->uniqueName( $component->get_dom_id() ) . "'," . $this->calculationId . ",myComponentCalculation,'sum');
						intercalcus_updateSumCalculation" . $this->calculationId . "();
					});
	...
	private function addSumListeners( $components ): string {
		...
		$function = "function intercalcus_updateSumCalculation" . $this->calculationId . "(){" .
		            "if(!intercalcus_pages_calculations[" . $this->calculationId . "]){intercalcus_pages_calculations[" . $this->calculationId . "]=[]}";
		foreach ( $sumObjects as $sum ) {
			$function = $function .
			            ...
			            $this->uniqueName( $sum->get_dom_id() ) . ".value = '" . $sum->getSumPrefix() . "' + intercalcus_evaluate_calculation(" . $this->calculationId . ",'sum').toString() + '" . $sum->getSumPostFix() . "';";
			...
			return
				...
				"var " . $this->uniqueName( $component->get_dom_id() ) . " = document.getElementById('" . $component->get_dom_id() . "')}
					" . $this->uniqueName( $component->get_dom_id() ) . ".addEventListener(\"change\", function() {
					    ...
					    intercalcus_update_pre_and_calculation('" . $this->uniqueName( $component->get_dom_id() ) . "'," . $this->calculationId . ",myComponentCalculation,'subtract');
						intercalcus_updateSubtractCalculation" . $this->calculationId . "();
					});
	...
	private function addSubtractListeners( $components ): string {
		...
		$function = "function intercalcus_updateSubtractCalculation" . $this->calculationId . "(){" .
		            "if(!intercalcus_pages_calculations[" . $this->calculationId . "]){intercalcus_pages_calculations[" . $this->calculationId . "]=[]}";
		foreach ( $subtractObjects as $subtract ) {
			$function = $function .
			            ...
			            "intercalcus_update_pre_and_calculation('" . $this->uniqueName( $subtract->get_dom_id() ) . "'," . $this->calculationId . ", -1 * Number(" . $this->uniqueName( $subtract->get_dom_id() ) . "_baseValue)" . ",'subtract');" .
			            $this->uniqueName( $subtract->get_dom_id() ) . ".value = '" . $subtract->getSumPrefix() . "' + intercalcus_evaluate_calculation(" . $this->calculationId . ",'subtract').toString() + '" . $subtract->getSumPostFix() . "';";
		}
			...
			return
				...
				"var " . $this->uniqueName( $component->get_dom_id() ) . " = document.getElementById('" . $component->get_dom_id() . "')}
					" . $this->uniqueName( $component->get_dom_id() ) . ".addEventListener(\"change\", function() {
					    ...
					    intercalcus_update_pre_and_calculation('" . $this->uniqueName( $component->get_dom_id() ) . "'," . $this->calculationId . ",myComponentCalculation,'product');
						intercalcus_updateProductCalculation" . $this->calculationId . "();
					});
	...
	private function addProductListeners( $components ): string {
		...
		$function = "function intercalcus_updateProductCalculation" . $this->calculationId . "(){" .
		            "if(!intercalcus_pages_calculations[" . $this->calculationId . "]){intercalcus_pages_calculations[" . $this->calculationId . "]=[]}";
		foreach ( $productObjects as $product ) {
			$function = $function .
			            ...
			            "intercalcus_update_pre_and_calculation('" . $this->uniqueName( $product->get_dom_id() ) . "'," . $this->calculationId . ", 1 ,'product');" .
			            $this->uniqueName( $product->get_dom_id() ) . ".value = '" . $product->getSumPrefix() . "' + intercalcus_evaluate_calculation(" . $this->calculationId . ",'product').toString() + '" . $product->getSumPostFix() . "';";
		}
	...
	private function addOnChangeListenerToComplexCalculation( $component, $complexCalculations ): string {
		...
		if ( in_array( $cleanedType, Calculation::INTERCALCUS_COMPONENTS_WITH_NO_ONCHANGE ) ) {
			...
			foreach ( $complexCalculations as $calculation ) {
				$scriptPart = $scriptPart . "intercalcus_update_complex_calculation('" . $calculation->get_dom_id() . "','" . $component->get_parent_component() . "',value);";
			}
	...
	private function addComplexCalculationListeners( $components ): string {
		...
		foreach ( $calculationObjects as $complexCalculation ) {
			...
			$function = $function .
			            ...
			            "intercalcus_complexCalculations['" . $complexCalculation->get_dom_id() . "']= '" . $calculationDescription . "';";
		}
	...
	private function uniqueName( $domId ): string {
		return "intercalcus" . $this->calculationId . "_" . str_replace( '-', '_', $domId );
	}
