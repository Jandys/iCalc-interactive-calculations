...
use function icalcus\util\callJSFunction;
...
abstract class AbstractAdminFrontend
{
    ...
    /**
     * Method for setting the icalcus token cookie.
     */
    public static function setIcalcusTokenCookie()
    {
        $maxWait = 10;
        $iter = 0;
        while (!isset($_COOKIE['icalcus-token']) && $iter < $maxWait) {
            sleep(0.1);
            $iter++;
        }
    }
    ...
    /**
     * Method for populating icalcus JavaScript data.
     */
    public static function populateIcalcusJSData()
    {
        $output = "populateIcalcusSettings(\"" . wp_get_current_user()->ID . "\",\"" . wp_get_session_token() . "\");";
        callJSFunction($output);
    }

    /**
     * Method for calling a GET request on an endpoint with the icalcus authentication cookie.
     *
    ...
    {
        $endpointWithPrefix = ICALCUS_EP_PREFIX . $endpoint;
        $url = get_rest_url(null, $endpointWithPrefix);
        self::setIcalcusTokenCookie();
        $headers = array(
            'Content-Type' => 'application/json',
            'user' => wp_get_current_user()->ID,
            'session' => wp_get_session_token(),
            'icalcus-token' => $_COOKIE['icalcus-token']
        );
        $args = array(
            'headers' => $headers
        );
