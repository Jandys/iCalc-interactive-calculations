...
const draggableComponents = document.querySelectorAll('.icalcus-draggable');
const dashboard = document.getElementById('icalcus-dashboard');
const dashboardEdit = document.getElementById('icalcus-dashboard-edit');
...
dashboard.addEventListener('dragstart', e => {
    draggedDashboardItem = e.target.closest('.icalcus-dashboard-item');
});
...
function addMovableButtons(cloneComponent, dashboardItem) {
    let configurationBar = cloneComponent.querySelector(".icalcus-configuration-bar");

    const moveUpButton = document.createElement('button');
    moveUpButton.innerHTML = '<i class="dashicons dashicons-arrow-up-alt"></i>';
    moveUpButton.classList.add("icalcus-config-btn");
    moveUpButton.classList.add("icalcus-btn-gray");
    ...
    moveDownButton.classList.add("icalcus-config-btn");
    moveDownButton.classList.add("icalcus-btn-gray");
...
dashboard.addEventListener('drop', e => {
    ...
    cloneComponent.classList.add("icalcus-configurable-draggable-option");
    ...
    dashboardItem.classList.add('icalcus-dashboard-item');
...
let icalcus_products;
let icalcus_services;
...
window.addEventListener('load', () => {
    const dashboardProducts = document.getElementById('icalcus-dashboard-products');
    ...
    productDiv.classList.add("icalcus-product-div");

    // Send an XHR request to get all products
    let productsXHR = icalcus_getAllProducts();
        ...
        if (productsXHR.readyState === XMLHttpRequest.DONE) {
            if (productsXHR.status === 200) {
                ...
                products.forEach(product => {
                    ...
                    span.classList.add("icalcus-selected-span-item");
 ...
 */
window.addEventListener('load', () => {
    const dashboardServices = document.getElementById('icalcus-dashboard-services');
    ...
    serviceDiv.classList.add("icalcus-service-div");

    // Send an XHR request to get all services
    let servicesXHR = icalcus_getAllServices();
...
const genericTypesLocalized = new Map(
    [[0, icalcusMainScriptLocalization.none],
        [1, icalcusMainScriptLocalization.label],
        [2, icalcusMainScriptLocalization.text],
        [3, icalcusMainScriptLocalization.list],
        [4, icalcusMainScriptLocalization.numberInput],
        [5, icalcusMainScriptLocalization.slider],
        [6, icalcusMainScriptLocalization.checkBox],
        [7, icalcusMainScriptLocalization.spacer],
        [8, icalcusMainScriptLocalization.horizontalRule]
    ]
);
...
const calculationTypesLocalized = new Map(
    [[0, icalcusMainScriptLocalization.none],
        [1, icalcusMainScriptLocalization.sum],
        [2, icalcusMainScriptLocalization.productCalculation],
        [3, icalcusMainScriptLocalization.subtractCalculation],
        [4, icalcusMainScriptLocalization.complexCalculation]
    ]);
 ...
 */
window.addEventListener('load', () => {
    const dashboardComponents = document.getElementById('icalcus-dashboard-components');
    ...
    componentDiv.classList.add("icalcus-component-div");
 ...
 */
window.addEventListener('load', () => {
    const dashboardCalculations = document.getElementById('icalcus-dashboard-calculations');
    ...
    calculationDiv.classList.add("icalcus-calculation-div");
...
function setNewServiceComponent(serviceComponent) {
    ...
    for (const child of serviceComponent.children) {
        ...
        if (child.id.startsWith("icalcus-dashboard-services")) {
            dashboard = child;
        }
    }
    ...
    if (select.children.length === 0) {
        ...
        dashboard.appendChild(noComponentFoundError(icalcusMainScriptLocalization.errorNoServiceFound));
    } else {
...
function setNewCalculationComponent(calculationComponent) {
    ...
    for (const child of calculationComponent.children) {
        modifyChildIdsWithSuffix(child, id);
        if (child.id.startsWith("icalcus-dashboard-calculations")) {
            dashboard = child;
        }
    }
    ...
    if (select.children.length === 1) {
        dashboard.removeChild(select);
        dashboard.appendChild(noComponentFoundError(icalcusMainScriptLocalization.errorNoCalculationComponentFound));
    } else {
        appendConfigButton(dashboard);
    }
...
function setNewGenericComponent(genericComponent) {
    ...
    for (const child of genericComponent.children) {
        modifyChildIdsWithSuffix(child, id);
        if (child.id.startsWith("icalcus-dashboard-components")) {
            dashboard = child;
        }
    }
...
function setNewProductComponent(productComponent) {
    ...
    for (const child of productComponent.children) {
        modifyChildIdsWithSuffix(child, id);
        if (child.id.startsWith("icalcus-dashboard-products")) {
            dashboard = child;
        }
    }
    ...
    if (select.children.length === 0) {
        dashboard.removeChild(select);
        dashboard.appendChild(noComponentFoundError(icalcusMainScriptLocalization.errorNoProductFound));
    } else {
        appendConfigButton(dashboard);
    }
...
function editUpdateObjectOfNewCalculation() {
    ...
    let calculationTitleObject = document.getElementById('icalcus-calulation-edit-name');
    let calculationTitle = calculationTitleObject.value ? calculationTitleObject.value : icalcusMainScriptLocalization.newCalcTitle;
...
async function createUpdateObjectOfNewCalculation() {
    ...
    let calculationTitleObject = document.getElementById('icalcus-calulation-new-name');
    let calculationTitle = calculationTitleObject.value ? calculationTitleObject.value : icalcusMainScriptLocalization.newCalcTitle;
    ...
    updateObject.customStyles = "";
    updateObject.configuration = currentCalculationConfiguration;
    await icalcus_getNextCalculationDescriptionId().then((value) => {
...
function getProductInHtml(product) {
    ...
    return "<table class='table table-bordered table-striped col-5 m-1'>" +
        "<thead>" +
        "<tr>" +
        "<th>" + icalcusMainScriptLocalization.id + "</th>" +
        "<th>" + icalcusMainScriptLocalization.name + "</th>" +
        "<th>" + icalcusMainScriptLocalization.description + "</th>" +
        "<th>" + icalcusMainScriptLocalization.pricePerUnit + "</th>" +
        "<th>" + icalcusMainScriptLocalization.unit + "</th>" +
        "<th>" + icalcusMainScriptLocalization.minQuantity + "</th>" +
        "<th>" + icalcusMainScriptLocalization.displayType + "</th>" +
        "</tr>" +
        "</thead>" +
        "<tbody>" +
        "<tr>" +
...
function getProductToJSONObject(productComponent) {
    ...
    let validProduct;
    for (const child of children) {
        if (child.id.startsWith("icalcus-dashboard")) {
            dashboard = child;
        }
    }
    ...
    if (validProduct !== undefined) {
        ...
        let elementNodeListOf = modalConf.querySelectorAll('.icalcus-custom-input');
...
function getConfigureModal(id, displayType) {
    return `
    <div class="icalcus-modal-wrapper hidden icalcus-component-configuration-modal">
        <div id="modal-${id}" class="icalcus-config-modal">
        <div class="modal-content p-3">
        <span>
          <h2>${icalcusMainScriptLocalization.personalCustomization}</h2>
        ...
        <span id="show-label-configuration" class="row m-0 align-items-center">
              <label class="col-2" for="show-label">${icalcusMainScriptLocalization.showLabel}:</label>
              <input type="checkbox" id="show-label" name="show-label" class="icalcus-custom-input form-check form-switch mb-2 ml-2 mr-4" data-previous=""/> 
        </span>
        
        <span class="display-none row m-0 align-items-center" id="custom-label-configuration">
              <label class="col-2" for="custom-label">${icalcusMainScriptLocalization.customLabel}:</label>
              <input type="text" id="custom-label" name="custom-label" class="icalcus-custom-input form-check form-switch mb-2 ml-2 mr-4" data-previous=""/> 
        </span>
         
         <span class="display-none row m-0 align-items-center" id="label-configuration" >
               <label for="label-classes">${icalcusMainScriptLocalization.labelClasses}:</label>
               <input type="text" id="label-classes" name="label-classes" class="icalcus-custom-input mt-0 mb-2 ml-4 mr-4" data-previous=""/> 
        </span>
        
         <span class="display-none row m-0 align-items-center" id="base-value-configuration" >
               <label for="base-value" class="col-2">${icalcusMainScriptLocalization.baseValue}:</label>
               <input type="number" id="base-value" name="base-value" class="icalcus-custom-input mt-0 mb-2 ml-4 mr-4" data-previous="" value="1"/> 
        </span>        
          
        <span class="display-none row m-0 align-items-center" id="unchecked-value-configuration" >
               <label for="unchecked-value" class="col-2">${icalcusMainScriptLocalization.uncheckedValue}:</label>
               <input type="number" id="unchecked-value" name="unchecked-value" class="icalcus-custom-input mt-0 mb-2 ml-4 mr-4" data-previous="" value="1"/> 
        </span>       
         
         <div class="display-none m-0 align-items-center" id="complex-calculation-configuration" >
               <label for="complex-calculation" class="col-4">${icalcusMainScriptLocalization.complexCalcConf}:</label>
               <textarea class="icalcus-custom-input col-10 mt-0 mb-4 ml-4 mr-4"  id="complex-calculation" name="complex-calculation" rows="3" cols="45" data-previous=""></textarea>
               <span class="mb-2 align-items-center">
                   <label for="complex-calculation-components">${icalcusMainScriptLocalization.complexCalcAddComp}:</label>
         ...
         <span class="display-none row m-0 align-items-center" id="subtract-calculation-configuration" >
               <label for="subtract-value" class="col-2">${icalcusMainScriptLocalization.subtractFromValue}:</label>
               <input type="number" id="subtract-value" name="subtract-value" class="icalcus-custom-input mt-0 mb-2 ml-4 mr-4" data-previous="" value="0"/> 
        </span>
        
        <span class="display-none row m-0 align-items-center" id="slider-configuration">
            <label class="col-2" for="slider-max">${icalcusMainScriptLocalization.sliderMax}:</label>
            <input type="number" id="slider-max" name="slider-max" class="icalcus-custom-input mt-0 mb-2 ml-4 mr-4" data-previous=""/>
            <label for="slider-show-value">${icalcusMainScriptLocalization.showValue}:</label>
            <input type="checkbox" id="slider-show-value" name="slider-show-value" class="icalcus-custom-input form-check form-switch mb-2 ml-2 mr-4" data-previous=""/>  
        </span>
        
        <span class="display-none" id="list-configuration" data-option="1">
            <span class="row m-0 align-items-center">
                <label class="col-2" for="list-option1">${icalcusMainScriptLocalization.listOption}:</label>
                <input type="text" id="list-option1" name="list-option1" class="icalcus-custom-input mt-0 mb-2 ml-4 mr-4 col-3" data-previous=""/>
                <label class="col-2" for="list-value1">${icalcusMainScriptLocalization.listValue}:</label>
                <input type="text" id="list-value1" name="list-value1" class="icalcus-custom-input mt-0 mb-2 ml-4 mr-4 col-3" data-previous=""/>
            </span>    
            
            <button id="list-add-option" class="icalcus-config-btn btn-info mt-2 close-btn icalcus-float-right"><i class="dashicons dashicons-plus-alt"></i></button>
        </span>
        
        <span class="display-none" id="sum-configuration">
            <span class="row m-0 align-items-center">
                <label class="col-2" for="list-option1">${icalcusMainScriptLocalization.sumPrefix}:</label>
                <input type="text" id="sum-prefix" name="sum-prefix" class="icalcus-custom-input mt-0 mb-2 ml-4 mr-4 col-3" data-previous=""/>
                <label class="col-2" for="sum-postfix">${icalcusMainScriptLocalization.sumSuffix}:</label>
                <input type="text" id="sum-postfix" name="sum-postfix" class="icalcus-custom-input mt-0 mb-2 ml-4 mr-4 col-3" data-previous=""/>
            </span>    
        </span>
         
          <span id="input-classes-configuration" class="row m-0 align-items-center">
               <label class="col-2" for="input-classes">${icalcusMainScriptLocalization.inputClasses}:</label>
               <input type="text" id="input-classes" name="input-classes" class="icalcus-custom-input mt-0 mb-2 ml-4 mr-4" data-previous=""/> 
          </span>
           
          <p class="font-italic font-weight-light text-info">${icalcusMainScriptLocalization.toAddMultipleClassesText}</p>
                
          <span id="custom-css-configuration">
             <label class="col-2"  for="custom-css">${icalcusMainScriptLocalization.customCss}:</label>
             <textarea class="icalcus-custom-input icalcus-custom-styler mt-0 mb-4 ml-4 mr-4"  id="custom-css" name="custom-css" rows="8" cols="50" placeholder=".myStyle{color:red}" data-previous=""></textarea>
          </span>
 ...
 */
function icalcus_getNewRowOfOptionsToConfigure(currentId) {
    const span = document.createElement('span');
    ...
    inputOption.className = "icalcus-custom-input mt-0 mb-2 ml-4 mr-4 col-3"
    ...
    inputValue.className = "icalcus-custom-input mt-0 mb-2 ml-4 mr-4 col-3"
...
function appendConfigButton(div) {
    ...
    button.className = 'icalcus-config-btn button';
    const componentType = icalcus_getComponentType(div.id.split("-")[2]);

    const id = "conf-" + div.parentNode.id;

    // Append the button to the div
    let configureDiv = div.parentNode.querySelector(".icalcus-configuration-bar");
    ...
    addListOptionBtn.onclick = () => {
        let elementNodeListOf = addListOptionBtn.previousElementSibling.querySelectorAll(".icalcus-custom-input");
        for (const prevInput of elementNodeListOf) {
            if (!prevInput.value) {
                alert(icalcusMainScriptLocalization.errorFillPreviousOptions);
                return;
            }
        }
        let currentId = Number(addListOptionBtn.parentNode.dataset.option);
        const {span, nextId} = icalcus_getNewRowOfOptionsToConfigure(currentId);
    ...
    function addOptionsToComplexCalculation() {
        if (modal.dataset.type === 'complex calculation') {
            ...
            for (const item of currentDashboard.children) {
                ...
                if (!component.id.includes("calculation") && listenableDisplayTypes.includes(icalcus_getDisplayTypeOfSelectedChild(component))) {
                    const option = document.createElement("option");
    ...
    function saveModal() {
        ...
        let customInputs = modal.querySelectorAll('.icalcus-custom-input');
    ...
    function changeLabel() {
        ...
        if (showLabel.checked === true) {
            ...
            let labelConfigurations = wrappingSpan.querySelectorAll('.icalcus-custom-input');
    ...
    function resetState(displayType, previousType) {
        ...
        if (previousType) {
            if (displayType !== previousType) {
                ...
                let labelInputs = labelSpan.querySelectorAll('.icalcus-custom-input');
                labelInputs.forEach((input) => {
                    input.value = ""
                });

                let sliderConfigurations = sliderConfSpan.querySelectorAll('.icalcus-custom-input');
                ...
                let baseValueInputs = baseValueConfiguration.querySelectorAll('.icalcus-custom-input');
                baseValueInputs.forEach((input) => {
                    input.value = ""
                });

                let listInputs = listConfiguration.querySelectorAll('.icalcus-custom-input');
                listInputs.forEach((input) => {
                    input.value = ""
                });

                let labelConfigurations = showLabelSpan.querySelectorAll('.icalcus-custom-input');
                labelConfigurations.forEach((input) => {
                    input.value = ""
                });

                let subtractValue = subtractConfiguration.querySelectorAll('.icalcus-custom-input');
                subtractValue.forEach((input) => {
                    input.value = "0"
                });

                let complexInputs = complexCalcConfiguration.querySelectorAll('.icalcus-custom-input');
                complexInputs.forEach((input) => {
                    input.value = ""
                })

                let uncheckedValues = uncheckedValueConfiguration.querySelectorAll('.icalcus-custom-input');
...
function updateEditPreview(jsonBodyToUpdate) {
    masterUpdatePreview(jsonBodyToUpdate, 'icalcus-preview-edit')
}

function updatePreview(jsonBodyToUpdate) {
    masterUpdatePreview(jsonBodyToUpdate, 'icalcus-preview')
}
...
function masterUpdatePreview(jsonBodyToUpdate, previewId) {
    ...
    wrapperDiv.id = 'icalcus-preview-wrapper';
    ...
    icalcus_calculations.set(updateObject["id"], []);
    ...
    for (const component of updateObject["components"]) {
        ...
        form.appendChild(icalcus_displayComponent(component, updateObject["id"], updateObject["components"]));
    }


    let customStyles = icalcus_createCustomStyle(updateObject["customStyles"])
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    toggleBtn.addEventListener('click', () => {
        if (thirdDiv.classList.contains('visible')) {
            firstDiv.classList.remove('icalcus-hidden-slow');
            firstDiv.classList.add('visible');
            secondDiv.classList.remove('visible');
            thirdDiv.classList.remove('visible');
            secondDiv.classList.add('icalcus-hidden-slow');
            thirdDiv.classList.add('icalcus-hidden-slow');
            ...
            toggleBtn.classList.remove('icalcus-reappear');
            toggleBtn.classList.add('icalcus-reappear');

            //clear possible components/modals left from editing
            icalcus_clear_edit_calculation_data();
        ...
        if (firstDiv.classList.contains('visible')) {
            ...
            firstDiv.classList.add('icalcus-hidden-slow');
            thirdDiv.classList.remove('visible');
            thirdDiv.classList.add('icalcus-hidden-slow');
            secondDiv.classList.remove('icalcus-hidden-slow');
    ...
    saveCalculation.onclick = async () => {
        ...
        if (updateObjectOfNewCalculation['components'].length > 0) {
            let calcCreationXHR = icalcus_process_calculation_description_creation();
            ...
            alert(icalcusMainScriptLocalization.errorNoValidComponents);
        }
    }
    ...
    editCalculation.onclick = () => {
        ...
        if (updateObjectOfNewCalculation['components'].length > 0) {
            let calcCreationXHR = icalcus_process_calculation_description_edit();
    ...
    editConfiguration.onclick = () => {
        ...
        modal.onchange = () => {
            icalcus_process_calculation_configuration_change(modal);
        };
...
function icalcus_getComponentType(idPart) {
    switch (true) {
...
let currentCalculationConfiguration = {
    "show-title": true,
    "calculation-description": icalcusMainScriptLocalization.defaultDescription,
    "wrapper-classes": "",
}
 ...
 */
function icalcus_set_calculation_configuration_values(calculateConfiguration) {
 ...
 */
function icalcus_process_calculation_configuration_change(modal) {
    const showTitleCheckBox = modal.querySelector('#show-title');
 ...
 * @returns {void}
 */
function icalcus_handle_button_and_div_swap() {
    const toggleBtn = document.getElementById('toggleBtn');
    const firstDiv = document.getElementById('firstDiv');
    const secondDiv = document.getElementById('secondDiv');
    const thirdDiv = document.getElementById('thirdDiv');
    ...
    thirdDiv.classList.remove("icalcus-hidden-slow");
...
function icalcus_set_edit_calculation_name(configuredName) {
    const input = document.getElementById("icalcus-calulation-edit-name");
    input.value = configuredName;
}

function icalcus_add_custom_styles_edit_calculation(customStyles) {
    let thirdDiv = document.getElementById('thirdDiv');
 ...
 * @return {void}
 * */
function icalcus_fill_components_of_edit_calculation(components) {
    const nextIds = {
        "product-component": 1,
        "service-component": 1,
    ...
    for (const component of components) {
        ...
        icalcus_insertDataToComponent(insertedComponent, component, id);
    }
...
function icalcus_insertDataToComponent(insertedComponent, jsonData) {
    ...
    icalcus_insertDataToComponentModal(insertedComponent, jsonData);
}
...
function icalcus_insertDataToComponentModal(insertedComponent, jsonData) {
        ...
        if (jsonData.displayType.toLowerCase() === 'list') {
            if (attribute.startsWith('list-')) {
                ...
                if (!possibleElement) {
                    ...
                    let {span, nextId} = icalcus_getNewRowOfOptionsToConfigure(id);
 ...
 */
function icalcus_process_calculation_edit_action(id) {
    const calculationXHR = icalcus_getCalculationDescriptionById(id);
    ...
    calculationXHR.onreadystatechange = function () {
        if (calculationXHR.readyState === XMLHttpRequest.DONE) {
            if (calculationXHR.status === 200) {
                ...
                icalcus_clear_create_calculation_data();

                icalcus_set_calculation_configuration_values(calculationObject.configuration);
                icalcus_handle_button_and_div_swap();

                icalcus_set_edit_calculation_name(calculationObject.title);
                icalcus_add_custom_styles_edit_calculation(calculationObject.customStyles);

                icalcus_fill_components_of_edit_calculation(calculationObject.components);
...
function icalcus_clear_edit_calculation_data() {
    const dashboard = document.getElementById('icalcus-dashboard-edit');
    dashboard.innerHTML = "";
    const preview = document.getElementById('icalcus-preview-edit');
    preview.innerHTML = "";
    icalcus_clear_configure_modals();
}

function icalcus_clear_create_calculation_data() {
    const dashboard = document.getElementById('icalcus-dashboard');
    dashboard.innerHTML = "";
    const preview = document.getElementById('icalcus-preview');
    preview.innerHTML = "";
    icalcus_clear_configure_modals();
}

function icalcus_clear_configure_modals() {
    let modals = document.body.querySelectorAll(".icalcus-component-configuration-modal");
    for (const modal of modals) {
        document.body.removeChild(modal.parentNode);
    }
}

function icalcus_getDisplayTypeOfSelectedChild(component) {
    ...
    switch (true) {
        case querySelector.name.includes("product"):
            return icalcus_products[selectedIndex - 1]["display_type"];
        case querySelector.name.includes("service"):
            return icalcus_services[selectedIndex - 1]["display_type"];
