...
let icalcus_calculations = new Icalcus_ObservableArray([]);
let icalcus_calculationElements = [];
let icalcus_calculationElementConfigurations = [];
icalcus_calculations.addListener((action, args, calculationId) => {
    if (icalcus_calculationElements[calculationId]) {
        for (const element of icalcus_calculationElements[calculationId]) {
            let prefix = icalcus_calculationElementConfigurations[calculationId]['sum-prefix']
            let postfix = icalcus_calculationElementConfigurations[calculationId]['sum-postfix']
            element.value = prefix + icalcus_calculate(calculationId, element).toString() + postfix;
        }
    }
});


function icalcus_getProductById(id) {
    const xhr = new XMLHttpRequest();
    const url = `/wp-json/icalcus/v1/products/${id}`;
    xhr.open('GET', url);
...
function icalcus_getServiceById(id) {
    const xhr = new XMLHttpRequest();
    const url = `/wp-json/icalcus/v1/services/${id}`;
    xhr.open('GET', url);
    xhr.setRequestHeader('Content-Type', 'application/json');
    return xhr;
}
...
function icalcus_getCalculationDescriptionById(id) {
    const xhr = new XMLHttpRequest();
    const url = `/wp-json/icalcus/v1/icalcusulation-descriptions/${id}`;
    xhr.open('GET', url);
    xhr.setRequestHeader('Content-Type', 'application/json');
    return xhr;
}
...
function icalcus_displayComponent(component, calculationId, components) {
    switch (component.type) {
        case "product":
            return icalcus_displayProduct(component, calculationId);
        case "service":
            return icalcus_displayService(component, calculationId);
        case "genericComponent":
        case "calculationComponent":
            return icalcus_displayGenericComponent(component, calculationId, components);
        default:
...
function icalcus_displayProduct(product, calculationId) {
    const productDiv = document.createElement("div");
    productDiv.classList.add("form-group");
    productDiv.classList.add("row");

    let productXHR = icalcus_getProductById(product.id);
    ...
    productXHR.onreadystatechange = function () {
        if (productXHR.readyState === XMLHttpRequest.DONE) {
            if (productXHR.status === 200) {
                ...
                productDiv.appendChild(icalcus_getDisplayType(product, productData, calculationId));
...
function icalcus_displayService(service, calculationId) {
    const serviceDiv = document.createElement("div");
    serviceDiv.classList.add("form-group");
    serviceDiv.classList.add("row");

    let serviceXHR = icalcus_getServiceById(service.id);
    ...
    serviceXHR.onreadystatechange = function () {
        if (serviceXHR.readyState === XMLHttpRequest.DONE) {
            if (serviceXHR.status === 200) {
                ...
                serviceDiv.appendChild(icalcus_getDisplayType(service, productData, calculationId));
...
function icalcus_displayGenericComponent(genericComponent, calculationId, components) {
    return icalcus_getDisplayType(genericComponent, {}, calculationId, components);

}


function icalcus_createCustomStyle(customStyles) {
...
function icalcus_getDisplayType(component, componentData, calculationId, components) {
    switch (component.displayType.toLowerCase()) {
        case "number":
        case "number input":
            return icalcus_getNumberDisplayType(component, componentData, calculationId);

        case "slider":
        case "range":
            return icalcus_getSliderDisplayType(component, componentData, calculationId);

        case "label":
            return icalcus_getLabelDisplayType(component, componentData, calculationId);

        case "horizontal rule":
        case "hr":
            return icalcus_getHorizontalRule(component);

        case "sum":
            return icalcus_getSumDisplayType(component, calculationId);

        case "subtract calculation":
        case "subtract-calculation":
            return icalcus_getSubtractCalculation(component, calculationId);

        case "product calculation":
        case "product-calculation":
            return icalcus_getProductCalculationDisplayType(component, calculationId);

        case "text":
            return icalcus_getTextDisplayType(component, componentData, calculationId);

        case "checkbox":
            return icalcus_getCheckboxDisplayType(component, componentData, calculationId);

        case "list":
            return icalcus_getListDisplayType(component, componentData, calculationId);

        case "spacer":
            return icalcus_getSpacerDisplayType(component);

        case "complex calculation":
            return icalcus_getComplexCalculationDisplayType(component, componentData, components);
...
function icalcus_getSumDisplayType(component, calculationId) {
    ...
    icalcus_calculationElementConfigurations[calculationId] = component.conf.configuration;

    if (!icalcus_calculationElements[calculationId]) {
        icalcus_calculationElements[calculationId] = [];
    }
    icalcus_calculationElements[calculationId].push(inputElement);
...
function icalcus_getProductCalculationDisplayType(component, calculationId) {
    const wrapper = document.createElement("div");
    const colLabel = document.createElement("div");
    colLabel.classList.add("col");
    colLabel.classList.add("form-label");
...
function icalcus_getSubtractCalculation(component, calculationId) {
    const wrapper = document.createElement("div");
    const colLabel = document.createElement("div");
    colLabel.classList.add("col");
    colLabel.classList.add("form-label");
...
function icalcus_getComplexCalculationDisplayType(component, componentData, components) {
    const wrapper = document.createElement("div");
    const colLabel = document.createElement("div");
    colLabel.classList.add("col");
    colLabel.classList.add("form-label");
            ...
            if (!replaceableComponent.domId.includes("calculation") && listenableDisplayTypes.includes(replaceableComponent.displayType.toLowerCase())) {
                ...
                if (calculationDescription.includes('[' + replaceableComponent.parentComponent + ']')) {
                    ...
                    inputElement.addEventListener('change', function () {
                        ...
                        icalcus_update_complex_calculation(`${component.type}-${component.id}-complex-calculation`, replaceableComponent.parentComponent, value);
...
function icalcus_update_complex_calculation(complexCalcId, component, value) {
    ...
    resultInput.value = resultInput.dataset.prefix + eval(icalcus_make_string_viable_for_eval(calculation)) + resultInput.dataset.sufix;
}

let complexCalculations = {};
let listenableDisplayTypes = ['list', "number", "number input", "slider", "checkbox"]

function icalcus_calculate(idOfCalculation, calculationElement) {
    let result = 0;
    ...
    const calculationObject = icalcus_calculations.get(idOfCalculation);
    for (const calcPart in calculationObject) {
        switch (true) {
            case calculationElement.id.includes("sum"):
                result += icalcus_simpleCalculation(calculationObject[calcPart]);
                break;
 ...
 */
function icalcus_simpleCalculation(calculationPart) {
    let preCalc = eval(calculationPart["baseValue"] * calculationPart["times"]);
    return calculationPart["negative"] ? eval(preCalc * -1) : preCalc;
}


function icalcus_getNumberDisplayType(component, componentData, calculationId) {
    ...
    colInput.onchange = () => {
        ...
        icalcus_calculations.setFrom(calculationId, `${component.domId}-number`, inputCalculation);
    }
...
function icalcus_getSliderDisplayType(component, componentData, calculationId) {
    const showLabel = component.conf.configuration["show-label"];
    const wrapper = document.createElement("div");
    if (showLabel === "true") {
        const colLabel = document.createElement("div");
    ...
    displayValue.classList.add("icalcus-display-slider-value");
    ...
    colInput.onchange = () => {
        ...
        icalcus_calculations.setFrom(calculationId, `${component.domId}-slider`, inputCalculation);
    }
...
function icalcus_getCheckboxDisplayType(component, componentData, calculationId) {
    const showLabel = component.conf.configuration["show-label"];
    const wrapper = document.createElement("div");
    if (showLabel === "true") {
        const colLabel = document.createElement("div");
    ...
    colInput.onchange = () => {
        ...
        icalcus_calculations.setFrom(calculationId, `${component.domId}-checkbox`, inputCalculation);
    }
...
function icalcus_getTextDisplayType(component, componentData, calculationId) {
    const showLabel = component.conf.configuration["show-label"];
    const wrapper = document.createElement("div");
    if (showLabel === "true") {
        const colLabel = document.createElement("div");
...
function icalcus_getLabelDisplayType(component) {
    const label = document.createElement("label")
    label.textContent = component.conf.configuration["custom-label"];
    return label;
}

function icalcus_getHorizontalRule(component) {
...
function icalcus_getSpacerDisplayType(component) {
...
function icalcus_getListDisplayType(component, componentData, calculationId) {
    const showLabel = component.conf.configuration["show-label"];
    const wrapper = document.createElement("div");
    if (showLabel === "true") {
        const colLabel = document.createElement("div");
