...
function issue_jwt_token($user_id, $session): string
{
    ...
    delete_site_transient('icalcus-secret-' . $user_id . $session);
    $randomCharacters = wp_generate_password();
    set_site_transient('icalcus-secret-' . $user_id . $session, $randomCharacters, 60 * 60);
    return $token;
}
...
function validate_jwt_token($token, $userid, $session)
{
    try {
        ...
        if ($userid == $uid && $sessionId == $session) {
            ...
            $secret = get_site_transient('icalcus-secret-' . $userid . $sessionId);
            if (empty($secret)) {
                return false;
            }
